...
export async function generateContentsJson(params: GenerateContentsJsonParameters): Promise<string | undefined> {
    ...
    const generateContentsCallback = async (rootPath: string, targetPath: string): Promise<Entry[]> => {
        const resultEntries: Entry[] = [];
        const folderItems = await FileSystem.readFolderItemNamesAsync(targetPath, { absolutePaths: true });
    ...
    const entries = await generateContentsCallback(params.targetPath, params.targetPath);
    const contentsData: ContentsData = { content: entries };

    return JSON.stringify(contentsData, null, 4);
}

/**
 * At a given target, runs build script to generate a contents.json for a behavior_pack or resource_pack.
 */
export function generateContentsJsonTask(params: GenerateContentsJsonParameters) {
    return async () => {
        const contentsJson = await generateContentsJson(params);
        if (!contentsJson) {
            return;
        }

        // Write the contents.json file.
        await FileSystem.writeFileAsync(params.outputFile, contentsJson, { ensureFolderExists: true });
    };
}
